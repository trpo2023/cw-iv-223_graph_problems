...
// заполнение матрицы смежности
	...
	for (int i = 0; i < M; i++)
	{
		printf("Введите начальную вершину, конечную и длину пути (например: 1 4 10): ");
		int l;
		scanf("%d %d %d", &a, &b, &l);
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			if (adj_matrix[i][j] != 0) {
				if (node_array[j].lp_len < node_array[i].lp_len + adj_matrix[i][j]) {
					node_array[j].lp_len = node_array[i].lp_len + adj_matrix[i][j];
					node_array[j].lp_count = node_array[i].lp_count;
				}
				else if (node_array[j].lp_len == node_array[i].lp_len + adj_matrix[i][j]) {
					node_array[j].lp_count += node_array[i].lp_count;
				}
			}
		}
...
void node_init(struct node* new_node, int is_s) { //создание нового узла, если ошибка, то возвращается NULL
	if (new_node != NULL) {
		if (is_s) {
			new_node->lp_len = 0;
			new_node->lp_count = 1;
			...
			new_node->lp_len = INT_MIN;     //максимальная длина равна самому минимальному int
			new_node->lp_count = 0;
		}
	};
}
